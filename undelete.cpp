...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	drive_z=*drivez;
	for (i=0;;i++){
		dirtmp=Get_Directory_Entry_Item(drivez,i,hDevice);
		dirrec=*dirtmp;
		if (strcmp(dirrec.DIR_Name,dname)){
			break;
		}
	}
	txtDevice=CreateFile(_T("F:\\sample.jpg"),GENERIC_READ | GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,NULL,CREATE_ALWAYS,0,NULL);
	if (txtDevice == INVALID_HANDLE_VALUE){
		return 1;
	}
	startclu=dirrec.DIR_FstClusHI*65535+dirrec.DIR_FstClusLO;
	/*SetFilePointer(hDevice,(drivez->BPB_ResvdSecCnt)*512,NULL,FILE_BEGIN);
	ReadFile(hDevice,buffer,512,&tempdw,NULL);
	fat32=(UNIT32 *)buffer;
	*(fat32+(dirtmp->DIR_FstClusLO))==0xffffff0f*/
	SetFilePointer(hDevice,(GET_FAT_FIRST_DATA_SECTOR(*(drivez))+(drivez->BPB_SecPerClus)*(startclu-2))*512,NULL,FILE_BEGIN);
	
	for (i=0;i<(dirrec.DIR_FileSize/2048);i++){
		SetFilePointer(hDevice,(GET_FAT_FIRST_DATA_SECTOR(*(drivez))+(drivez->BPB_SecPerClus)*(startclu-2))*512+i*2048,NULL,FILE_BEGIN);
		ReadFile(hDevice,buffer2,2048,&tempdw,NULL);
		//SetFilePointer(txtDevice,i*2048,NULL,FILE_BEGIN);
		WriteFile(txtDevice,buffer2,2048,&tempdw,NULL);
	}
	if (dirrec.DIR_FileSize%2048 != 0){
		SetFilePointer(hDevice,(GET_FAT_FIRST_DATA_SECTOR(*(drivez))+(drivez->BPB_SecPerClus)*(startclu-2))*512+i*2048,NULL,FILE_BEGIN);
		ReadFile(hDevice,buffer2,dirrec.DIR_FileSize%2048,&tempdw,NULL);
		//SetFilePointer(txtDevice,i*2048,NULL,FILE_BEGIN);
		WriteFile(txtDevice,buffer2,dirrec.DIR_FileSize%2048,&tempdw,NULL);
	}
	return 0;
}
